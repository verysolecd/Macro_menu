...
'On Error Resume Next ' 临时开启错误处理
 Err.Number = 0
 Dim g_frm: Set g_frm = KCL.newFrm(mdlname): g_frm.Show
 If LCase(g_frm.BtnClicked) <> "btnok" Then Exit Sub
    Dim oDoc: Set oDoc = CATIA.ActiveDocument
    Dim opath As String: opath = ""
    If g_frm.res("chk_path") Then
    ...
    If opath = "" Then Exit Sub
        ...
        DraftMgr.Commit
   If g_frm.res("chk_save") Then oDoc.Save
