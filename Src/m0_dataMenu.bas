...
'-----------------------------------------------------------------
' 菜单入口点
Private Const mdlname As String = "m0_dataMenu"
Public Sub CATMain()
    Set PageMap = get_Tagcfg(groupName, True)  '获取page编号和名称对应map  ：1  R&W 2...showdict PageMap
    ...
    Dim SoLst As Object:  Set SoLst = To_SortedList(ButtonInfos)   ' 对按钮信息进行排序
    If SoLst Is Nothing Then Exit Sub
    ' 显示菜单界面
    Dim Menu: Set Menu = New Cat_Macro_Menu_View
...
Private Function Get_ButtonInfo() As Object
    ...
    Dim PjtPath As String: PjtPath = ExecPjt.DisplayName ' ==   获取catvba文件path
    ' ==  获取所有Module，这里还未识别是否是菜单里的按钮
...
For Each comp In AllComps
    ...
    Set MdlInfo = get_Tagcfg(DecCode) ' 将声明与page pair对比，获取配置信息
    ...
    Else
        GoTo Continue
    End If '  Debug.Print TypeName(MdlInfo(TAG_GROUP)) & " : " & MdlInfo(TAG_GROUP)
    If Not PageMap.Exists(MdlInfo(TAG_GROUP)) Then GoTo Continue '若mpages编号不包含该分组值则下一个
    CanExecMethod = vbNullString ' 检查入口点方法
...
Try_TAG_ENTRY_DEF:
    ...
    End If
        ...
        BtnInfos.Add MdlInfo  '   showdict (MdlInfo)
Continue:
Next
...
' 参数  : str,Opt_bool
' 返回值: Dict
Private Function get_Tagcfg( _
                    ByVal txt As String, _
                    Optional ByVal KeyToLong As Boolean = False) _
                    As Object
    Set get_Tagcfg = Nothing
    ...
    For Each match In matches
        ...
        Set Dic = Push_Dic(Dic, key, Var)  '获取key—var dic对
Continue:
    Next
...
Private Function Exist_Method(ByVal codemdl As Object, _
    ...
    Dim tmp As Long: tmp = codemdl.ProcBodyLine(Name, 0)
    On Error GoTo 0
    Exist_Method = tmp > 0
    Err.Number = 0
End Function
' 获取标准模块列表
' 参数 itms : obj-VBComponents
