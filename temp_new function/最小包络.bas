...
'äÓò∞éwíËìI?ïi?åöMinimumBox

Option Explicit

Private Const MINBODYNAME = "MinimumBox" 'MinimumBoxñºèÃ
Private Const DMYLNG = 1000000# 'ãï?ñ ãó?
Private Enum MINMAX '?ó ?êî?çıà¯óp
    MinX = 0
    MaxX = 1
    MinY = 2
    MaxY = 3
...
Private Function getMaxSize_Bodies( _
    ...
    '?ó ï˚å¸ópå¸ó  êî?çıà¯égópEnum MinMax
    Dim vec As Variant
    vec = Array( _
        Array(-1#, 0#, 0#), _
        Array(1#, 0#, 0#), _
...
' ?éÊçø?ånBrepName
' PlaneN0 : 0-XY,1-YZ,2-ZYíÜìI?àÍò¢
    ...
    Select Case True
        Case Abs(posSt(0) - posEd(0)) < 0.001
            Call initConstraint( _
                ...
                line, ax2D.VerticalReference) 'ëÊ3,4éQêî?èòïsî\?ì|
        ...
        Case Abs(posSt(1) - posEd(1)) < 0.001
            Call initConstraint( _
                ...
                line, ax2D.HorizontalReference) 'ëÊ3,4éQêî?èòïsî\?ì|
